...
* // returns 1
*/
function zindexOfColumn( order, M, N, A, LDA, x, strideX, workspace, strideW ) {
	var sa1;
	var sa2;
	var s;
	var l;
	...
	if ( l === null ) {
		throw new TypeError( format( '2itFx', order ) );
	}
	if ( isRowMajor( l ) ) {
		s = N;
		sa1 = LDA;
	...
	if ( LDA < max( 1, s ) ) {
		throw new RangeError( format( '2itIR', max( 1, s ), LDA ) );
	}
	return ndarray( M, N, A, sa1, sa2, 0, x, strideX, stride2offset( M, strideX ), workspace, strideW, stride2offset( N, strideW ) ); // eslint-disable-line max-len
}
