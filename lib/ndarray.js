...
*
* // A (row-major) => [ [ 1+0i, 2+0i ], [ 3+0i, 4+0i ], [ 0+0i, 0+0i ] ]
...
*
* var out = zindexOfColumn( 3, 2, A, 2, 1, 0, x, 1, 0, workspace, 1, 0 );
* // returns 1
*/
function zindexOfColumn( M, N, A, strideA1, strideA2, offsetA, x, strideX, offsetX, workspace, strideW, offsetW ) {
	var abuf;
	var xbuf;
	var da0;
	var da1;
	...
	if ( isColumnMajor( [ strideA1, strideA2 ] ) ) {
		...
		for ( i1 = 0; i1 < S1; i1++ ) {
			...
			ix = offsetX;
			for ( i0 = 0; i0 < S0; i0++ ) {
				if ( abuf[ ia ] !== xbuf[ ix ] || abuf[ ia+1 ] !== xbuf[ ix+1 ] ) {
